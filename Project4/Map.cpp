...
void Map::updateDeathRoom() {
    int currDeathRoom = -1;  // Initialize to an invalid value
    // Find the current death room
    for (int i = 0; i < areasVec.size(); i++) {
        if (areasVec[i]->info.getID() == true) {
            currDeathRoom = i;
            break;  // Stop loop once we find the current death room
        }
    }

    // If a death room was found, make it safe
    if (currDeathRoom != -1) {
        areasVec[currDeathRoom]->info.setID(0);
        //cout << "Old death room " << (currDeathRoom + 1) << " is now safe." << endl;
    }

    // Select a random death room
    int newDeathRoom = rand() % areasVec.size();

    // Set the new death room
    areasVec[newDeathRoom]->info.setID(1);
    //
    //cout << "New death room is " << (newDeathRoom + 1) << "." << endl;
}

vector<areaNode*> Map::getAreaVec()
{
    return areasVec;
}
